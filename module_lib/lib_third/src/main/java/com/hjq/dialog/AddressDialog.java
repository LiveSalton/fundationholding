    ...
    public static final class Builder
            ...
            mCallback = new ViewPager2.OnPageChangeCallback() {
                ...
                public void onPageScrollStateChanged(int state) {
                    ...
                    if (state == ViewPager2.SCROLL_STATE_IDLE &&
                            mTabLayout.getSelectedTabPosition() != mViewPager.getCurrentItem()) {
        ...
        public Builder setIgnoreArea() {
            ...
            if (data != null && !data.isEmpty()) {
                // 已经指定了城市，不能再忽略县级区域
                throw new IllegalStateException(
                        "Cities have been designated and county-level areas can no longer be ignored");
            }
            mIgnoreArea = true;
            return this;
        }
        ...
        public void onSelected(int recyclerViewPosition, int clickItemPosition) {
            switch (recyclerViewPosition) {
                case 0:
                    ...
                    mTabLayout.addTab(mTabLayout.newTab().setText(getString(R.string.address_hint)), true);
                    mAdapter.addItem(
                            AddressManager.getCityList(
                                    mAdapter.getItem(recyclerViewPosition).get(clickItemPosition).getNext()));
                ...
                case 1:
                    ...
                    if (mIgnoreArea) {
                        ...
                        mTabLayout.addTab(mTabLayout.newTab().setText(getString(R.string.address_hint)), true);
                        mAdapter.addItem(AddressManager
                                .getAreaList(mAdapter.getItem(recyclerViewPosition).get(clickItemPosition).getNext()));
                        mViewPager.setCurrentItem(recyclerViewPosition + 1);
                    }
