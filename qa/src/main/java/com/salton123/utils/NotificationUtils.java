...
public class NotificationUtils {
     ...
     * @param summary     内容
     */
    public static void setMessageNotification(Context context, int notifyId, int smallIconId, CharSequence title,
     ...
     */
    public static void setMessageNotification(Context context, int notifyId, int smallIconId, CharSequence title,
                                              CharSequence summary, CharSequence ticker) {
     ...
     */
    public static void setMessageNotification(Context context, int notifyId, int smallIconId, CharSequence title,
                                              CharSequence summary, CharSequence ticker, PendingIntent pendingIntent) {
        ...
        builder.setSmallIcon(smallIconId)
                .setLargeIcon(BitmapFactory.decodeResource(context.getResources(), R.drawable.default_icon))
     ...
     * @param pendingIntent 点击后的intent
     */
    public static void setInfoNotification(Context context, int notifyId, CharSequence title, CharSequence summary,
                                           CharSequence ticker, PendingIntent pendingIntent) {
        NotificationCompat.Builder builder;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            builder = new NotificationCompat.Builder(context, ID_HIGH_CHANNEL);
        } else {
     ...
     * @param progress 进度（0-100）
     */
    public static void setProgressNotification(Context context, int notifyId, CharSequence title, CharSequence ticker,
                                               int progress) {
     ...
     */
    public static void setProgressNotification(Context context, int notifyId, CharSequence title, CharSequence ticker,
                                               int progress, PendingIntent pendingIntent) {
        NotificationCompat.Builder builder;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            builder = new NotificationCompat.Builder(context, ID_HIGH_CHANNEL);
        } else {
    ...
    private static NotificationManager createNotificationManager(Context context) {
        ...
        // 适配>=7.0手机通知栏显示问题
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationChannel notificationHightChannel =
                    new NotificationChannel(ID_HIGH_CHANNEL, NAME_HIGH_CHANNEL, NotificationManager.IMPORTANCE_HIGH);
            NotificationChannel notificationLowChannel =
                    new NotificationChannel(ID_LOW_CHANNEL, NAME_LOW_CHANNEL, NotificationManager.IMPORTANCE_LOW);
