package com.salton123.qa.kit.network.core;
...
import com.salton123.qa.kit.network.bean.Request;
import com.salton123.qa.kit.network.bean.Response;
import com.salton123.qa.kit.network.stream.InputStreamProxy;
import com.salton123.qa.kit.network.utils.Utf8Charset;
...
public class NetworkInterpreter {
    ...
    public void httpExchangeFailed(int requestId, String s) {
        XLog.i(TAG, "[httpExchangeFailed] requestId: " + requestId + " error: " + s);
    }
    ...
    public void responseReadFinished(int requestId, NetworkRecord record, ByteArrayOutputStream outputStream) {
        if (outputStream != null) {
            ...
            XLog.i(TAG, "[responseReadFinished] body: " + record.mResponseBody.toString().length());
        } else {
            XLog.i(TAG, "[responseReadFinished] outputStream is null request id: " + requestId);
        }
    }

    public void responseReadFailed(int requestId, String s) {
        XLog.i(TAG, "[responseReadFailed] requestId: " + requestId + " error: " + s);
    }
