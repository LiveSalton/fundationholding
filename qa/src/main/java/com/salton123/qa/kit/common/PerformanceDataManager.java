package com.salton123.qa.kit.common;
...
import com.salton123.qa.kit.custom.UploadMonitorInfoBean;
import com.salton123.qa.kit.custom.UploadMonitorItem;
import com.salton123.qa.kit.network.NetworkManager;
import com.salton123.utils.ActivityUtils;
import com.salton123.utils.FileManager;
...
public class PerformanceDataManager {
    ...
    private void executeCpuData() {
        XLog.d(TAG, "current thread name is ==" + Thread.currentThread().getName());
        if (mAboveAndroidO) {
            mLastCpuRate = getCpuDataForO();
            XLog.d(TAG, "cpu info is =" + mLastCpuRate);
    ...
    private void executeMemoryData() {
        ...
        XLog.d(TAG, "memory info is =" + mLastMemoryInfo);
    ...
    private void saveToLocal() {
        ...
        if (ActivityUtils.getCurrentResumedActivity() != null) {
            pageName = ActivityUtils.getCurrentResumedActivity().getLocalClassName();
        }
        info.page = pageName;
    ...
    private void uploadDataToLocalFile() {
        Observable.create((ObservableOnSubscribe<Void>) emitter -> FileManager
                .writeTxtToFile(JsonUtil.jsonFromObject(mUploadMonitorBean), filePath, customFileName))
                .subscribe(aVoid -> {
                }, throwable -> XLog.e("PerformanceDataManager", throwable.getMessage()));
    }
    ...
    private float getCPUData() {
        ...
        try {
            ...
            XLog.e(TAG, "getCPUData fail: " + e.toString());
        }
        return value;
    }
    ...
    private float getMemoryData() {
        ...
        try {
            ...
            XLog.e(TAG, "getMemoryData fail: " + e.toString());
        }
        return mem;
    }
    ...
    private float parseCPUData(String data) throws IOException {
        ...
        while ((line = bufferedReader.readLine()) != null) {
            ...
            if (line.contains("Permission Denial")) {
                ...
                String[] lineItems = line.split("\\s+");
                if (lineItems != null && lineItems.length > 1) {
                    XLog.d(TAG, "result is ==" + lineItems[0]);
                    bufferedReader.close();
