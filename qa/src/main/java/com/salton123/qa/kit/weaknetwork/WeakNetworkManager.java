...
public class WeakNetworkManager {
    ...
    public UnknownHostException simulateOffNetwork(String host) {
        return new UnknownHostException(
                String.format("Unable to resolve host \"%s\": No address associated with hostname", host));
    }
    ...
    public SocketTimeoutException simulateTimeOut(String host, int port) {
        ...
        return new SocketTimeoutException(
                String.format("failed to connect to %s (port %d) after %dms", host, port, mTimeOutMillis));
    }
    ...
    public Response simulateSpeedLimit(Interceptor.Chain chain) throws IOException {
        ...
        if (body != null) {
            request = request.newBuilder().method(request.method(), new SpeedLimitRequestBody(mRequestSpeed, body))
                    .build();
        }
