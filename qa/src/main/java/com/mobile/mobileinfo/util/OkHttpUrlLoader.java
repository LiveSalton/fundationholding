...
public class OkHttpUrlLoader implements ModelLoader<String> {
    ...
    public void loadData(@NonNull DataCallback<? super String> callback) {
        ...
        if (RequestMethod.POST == httpModel.getRequestMethod() &&
                Preconditions.checkNotEmptyBoolean(httpModel.getPostParam().getStringParam())) {
            requestBuilder.post(RequestBody.create(MediaType.parse("application/octet-stream"),
                    httpModel.getPostParam().getStringParam().getBytes()));
        }
